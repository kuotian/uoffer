    ...
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="法人身份证号" align="center" prop="idCard" :show-overflow-tooltip="true" width="180"/>
          <el-table-column label="统一社会信用代码" align="center" prop="uscCode" :show-overflow-tooltip="true" width="180"/>
          ...
          <el-table-column label="是否可以发布信息" align="center" width="140">
            <template slot-scope="scope">
              <el-switch
                v-model="scope.row.isPublish"
                :active-value=true
                :inactive-value=false
                @change="handleChangePublish(scope.row)"
              ></el-switch>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <el-button
                ...
                v-hasPermi="['bus:reviewEnt:query']"
              >查看</el-button>
              <el-button
            size="mini"
            type="text"
...
import { listReview, reviewById, getById, changePublish, delById } from "@/api/enterprise";
...
export default {
  ...
  methods: {
    ...
    handleChangePublish(row) {
      let text = row.isPublish === true ? "不禁止" : "禁止";
      this.$confirm('确认要"' + text + '""' + row.username + '"发布消息吗?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return changePublish(row.id, row.isPublish);
        }).then(() => {
          this.msgSuccess(text + "成功");
        }).catch(function() {
          row.isPublish = row.isPublish === false ? true : false;
        });
    },
  }
};
